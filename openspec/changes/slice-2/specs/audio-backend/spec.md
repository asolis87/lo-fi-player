# audio-backend Specification (Delta vs Slice #1)

## Purpose

Defines the contract for `lofi play <id>` resolving the audio path of a catalog track and dispatching it to the mpv backend. This is a delta spec against the slice #1 audio-backend behavior, where the runtime path for catalog tracks short-circuited on `procedural:` and `lofi play <id>` passed `Path: ""` to the backend (commented as "wired later").
...
(Previously: `runHeadlessPlay` passed `Path: ""` to the backend. The code comment said "mpv.go will read the cache by track id when the audio path is wired in" — it was not wired.)
...
(Previously: only `procedural:rain` is whitelisted. The whitelist is enforced in `cmd/lofi/play.go`.)
...
(Previously: this contract was implemented; slice #2 re-states it for completeness.)
